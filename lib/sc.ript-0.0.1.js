(function() {
  var Bitmap, Button, ByteArray, CapsStyle, Color, DLoader, DisplayObject, Event, EventEmitter, GraphicsPathCommand, JointStyle, Matrix, NumberUtil, Point, Rectangle, Type, k, path, v, _ref,
    __slice = [].slice,
  ...
  Bitmap = (function(_super) {
    ...
    Bitmap.prototype.drawSpline = function(points, interpolation) {
      ...
      for (j = _i = 0; _i < jLen; j = _i += 1) {
        ...
        if (j === jLen - 1) {
          iLen = closed != null ? closed : {
            1: interpolation + 1
          };
        }
        for (i = _j = 0; _j < iLen; i = _j += 1) {
          commands.push(GraphicsPathCommand.LINE_TO);
          data.push(_interpolateSpline(p0.x, p1.x, p2.x, p3.x, i / interpolation), _interpolateSpline(p0.y, p1.y, p2.y, p3.y, i / interpolation));
        }
      }
    ...
    Bitmap.prototype._normalizeIndex = function(index, pointsLength, closed) {
      ...
      if (!closed) {
        ...
        } else if ((_ref = index >= pointsLength) != null ? _ref : 1 + (index - pointsLength)) {
          return index;
        }
      }
    };
